// Package ucichessplayer uci.go (partly) implements the UCI protocol for communicating with
// chess engines. It's depended on the chess and board representation of the github.com/notnil/chess package
package ucichessplayer
...
// Info struct to hold the inforation returned by the uci engine
type Info struct {
	Line     string
...
// Err exposes the internal Info.err
func (i Info) Err() error { return i.err }

// BestMove returns the bestmove when uci engine has returned that in the uci communication
...
//Pv returns the principal variation of the uci engine
func (i Info) Pv() *Pv {
	pv, ok := i.Value("pv")
	if !ok {
		return nil
	...
	for _, move := range fields {
		lan := chess.LongAlgebraicNotation{}
		m, err := lan.Decode(i.position, move) //m, err := b.ParseMove(move)
		if err != nil {
			break
		}
...
//Stats returns generic search statistics from the uci engine like searched depth, seldepth nodes and time taken.
